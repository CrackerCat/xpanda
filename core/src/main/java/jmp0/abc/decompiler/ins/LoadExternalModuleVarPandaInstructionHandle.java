 ...
 * @Email: jmp0@qq.com
 */
public final class LoadExternalModuleVarPandaInstructionHandle implements IInstructionHandle<LoadExternalModuleVarPandaInstruction>{
    @Override
    public HandleStatus handle(LoadExternalModuleVarPandaInstruction instruction, PandaIRBasicBlock block, PandaDecompilerMethodHandler methodHandler, PandaLexical pandaLexical, Object node) {
        CodeGenerator codeGenerator = methodHandler.getCodeGenerator();
        Object assignmentExpressionNode = codeGenerator.getNodeFactory().assignmentExpressionNode("=",codeGenerator.getNodeFactory().identifierNode("acc"),codeGenerator.getNodeFactory().identifierNode(instruction.getModule().getLocalName()));
